/*
	...
	Modified 2026-09 for usb-browser-for-android: the device I/O part is compiled out under USBFS_CUSTOM_IO (replaced by exfat_io.c of usb-browser-for-android).
*/
...
/* usb-storage plugin: the block device layer below is replaced by cpp/exfat_io.c
   (USB block device over JNI) when USBFS_CUSTOM_IO is defined. */
#ifndef USBFS_CUSTOM_IO
struct exfat_dev
{
	int fd;
