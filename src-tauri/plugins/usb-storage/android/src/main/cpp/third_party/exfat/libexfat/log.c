/*
	...
	Modified 2026-09 for usb-browser-for-android: under USBFS_CUSTOM_IO, exfat_error() also records the message for the app (usbfs_log_error).
*/
...
void exfat_error(const char* format, ...)
{
	...
	exfat_errors++;
	va_start(ap, format);
#ifdef USBFS_CUSTOM_IO
	/* usb-storage plugin: remember the last error for the UI */
	{
		extern void usbfs_log_error(const char* format, va_list ap);
		va_list ac;
		va_copy(ac, ap);
